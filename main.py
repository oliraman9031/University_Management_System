...
def manage_events():
    ...
    # Check if user is admin (assuming admin ID 1)
    if session['user'][0] != 1:
    ...
    try:
        ...
        for event in events:
            # Convert event data to list format expected by template
            event_list = [
                event['Event_ID'],
                event['Event_Name'],
                event['Event_Type'],
                event['Event_Date'].strftime('%Y-%m-%d') if hasattr(event['Event_Date'], 'strftime') else event['Event_Date'],
                event['Venue'],
                event['Status'],
                event.get('Organized_By', ''),
                event.get('Description', '')
            ]
            
            if event['Event_Date'] >= today and event['Status'] != 'Completed':
                upcoming_events.append(event_list)
            else:
                past_events.append(event_list)
    ...
    return render_template('manage_events.html', 
                         ...
                         message_danger=request.args.get('message_danger'))
...
def update_event(event_id):
    ...
    if request.method == 'POST':
        try:
            event_name = request.form.get(f'event_name_{event_id}')
            event_type = request.form.get(f'event_type_{event_id}')
            event_date = request.form.get(f'event_date_{event_id}')
            venue = request.form.get(f'venue_{event_id}')
            status = request.form.get(f'status_{event_id}')
            
            query = """UPDATE events 
                      SET Event_Name=%s, Event_Type=%s, Event_Date=%s, Venue=%s, Status=%s 
                      WHERE Event_ID=%s"""
            mycursor.execute(query, (event_name, event_type, event_date, venue, status, event_id))
            mydb.commit()
...
def delete_event(event_id):
    ...
    try:
        # First delete participation records
        mycursor.execute("DELETE FROM student_event_participation WHERE Event_ID=%s", (event_id,))
...
def register_event(event_id):
    ...
    try:
        ...
        mycursor.execute("SELECT * FROM student_event_participation WHERE Student_ID=%s AND Event_ID=%s", 
                        (student_id, event_id))
        existing = mycursor.fetchone()
        
        if not existing:
            mycursor.execute("INSERT INTO student_event_participation (Student_ID, Event_ID, Registration_Date) VALUES (%s, %s, NOW())", 
                           (student_id, event_id))
            mydb.commit()
...
# Add this route to view registered events for a student
# @app.route('/my_events')
# def my_events():
#     if 'user' not in session:
...
#     try:
#         query = """
#             SELECT e.*, sep.Registration_Date 
#             FROM events e
#             INNER JOIN student_event_participation sep ON e.Event_ID = sep.Event_ID
#             WHERE sep.Student_ID = %s
...
#     except Exception as e:
#         registered_events = []
#         flash(f'Error fetching your events: {str(e)}', 'danger')
    
#     return render_template('events.html', events=registered_events)
